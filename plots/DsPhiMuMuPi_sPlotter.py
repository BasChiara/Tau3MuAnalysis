...
def make_sPlot(
    ...
    tag = (f'_{args.tag}' if args.tag else '') + (f'_{add_tag}' if add_tag else '')
...
def reweight_by_eta():
    ...
    h_ratio = h_sData.Clone('h_ratio')
    h_ratio.Divide(h_mc)
    # print h_ratio content
    for i in range(1, h_ratio.GetNbinsX() + 1):
        print(f'bin {i}: {h_ratio.GetBinContent(i)}')

    
    return h_ratio

def apply_reweighting(observable, h_ratio, nbins = 30, lo = -3, hi = 3, selection = '1', to_ploton = [], add_tag = '', cat = 'ABC'):
    ...
    for i in range(mc_tree.GetEntries()):
        ...
        h_reweighted.Fill(
            ...
            h_ratio.GetBinContent(h_ratio.FindBin(getattr(mc_tree, 'Ds_fit_eta'))) * mc_tree.weight * mc_tree.norm_factor
        )

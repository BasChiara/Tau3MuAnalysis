...
void DsPhiMuMuPi_analyzer::Loop(){
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if (!( 
            ...
            DsPlusMET_Flag_BadPFMuonFilter &&
            DsPlusMET_Flag_EcalDeadCellTriggerPrimitiveFilter &&
            DsPlusMET_Flag_eeBadScFilter &&
            DsPlusMET_Flag_globalSuperTightHalo2016Filter &&
            DsPlusMET_Flag_goodVertices &&
            DsPlusMET_Flag_hfNoisyHitsFilter
      )) continue;
      nEvMETfilters++;
    ...
    std::cout << " Ds candidates MC matched " << nDsMCmatched << std::endl;
...
void DsPhiMuMuPi_analyzer::outTreeSetUp(){
    ...
    outTree_ = new TTree(outTree_name_, "DsPhiMuMuPi candidates with MC matching");
    ...
    // * MC matching
    outTree_->Branch("isMC",             &isMC_,           "isMC/I");

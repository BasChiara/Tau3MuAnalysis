...
combine_dir  = '/afs/cern.ch/user/c/cbasile/Combine_v10/CMSSW_14_1_0_pre4/src/WTau3Mu_limits/bias_study_v2/input_combine/'
...
for cat in working_points:
    print(f'\n\n------ CATEGORY {cat} ------\n\n')
    cmd = f'python3 Tau3Mu_fitSB.py -s {input_signal} -d {input_data} --plot_outdir {plot_outdir} --goff --save_ws --sys_unc --combine_dir {combine_dir} --tag {tag}_{b_func} -b {b_func} -c {cat} -y {year} --bdt_cut {working_points[cat]}'
    if args.dry_run:
        print(cmd)
    else:
        os.system(cmd)

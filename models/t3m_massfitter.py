...
if __name__ == "__main__":
    ...
    parser.add_argument('--multipdf',     action = 'store_true' ,                                             help='fit with multiple pdfs')    
    ...
    #    print(f'{ct.RED}[ERROR]{ct.END} no events in WTau3Mu MC with selection {sgn_selection}')
    #    exit()
    ...
    if not args.goff : fitu.draw_fit_pull(frame_W, fitvar=mass, out_name=f'{args.plot_outdir}/massfit_S_Wt3m_{point_tag}')
    ...
    if not args.goff: fitu.draw_fit_pull(frame_Z, fitvar=mass, out_name=f'{args.plot_outdir}/massfit_S_Zt3m_{point_tag}')
    ...
    if USE_CMS_STYLE:
        ...
        fitu.apply_cms_style(frame_b, 
                            ...
                            year='20'+args.year, 
                            Preliminary=True,
                            ymax=np.round(1.2*frame_b.GetMaximum()) if cut < 0.990 else 10,
                            )
    else:

...
cat_edges_dict = {
    ...
    'C' : np.linspace(config.eta_thBC, 2.5, Nsplits+1),
}
print(f'{ct.color_text.BOLD}Split categories in {Nsplits} bins{ct.color_text.END}')
print(f' > cat A edges {cat_edges_dict["A"]}')
print(f' > cat B edges {cat_edges_dict["B"]}')
print(f' > cat C edges {cat_edges_dict["C"]}')


wspace_filename = f'{args.combine_dir}/wspace_etaBinning_{tag}.root'
out_data_filename = f'{args.combine_dir}/etaBinning_TTree_{tag}.root'
...
print(f'{ct.color_text.RED}[+] added MC file :\n {mc_file}{ct.color_text.END}')
...
print(f'{ct.color_text.BLUE}[+] added DATA file :\n {mc_file}{ct.color_text.END}')
...
for cat in config.cat_eta_selection_dict_fit:
    if cat == 'ABC' : continue
    print(f'{ct.color_text.BOLD}[i] processing CAT {cat}{ct.color_text.END}\n')
    split_edges = [cat_edges_dict[cat][i:i+2] for i in range(len(cat_edges_dict[cat])-1)]
    for j, cat_p in enumerate(split_edges):
        
        print(f'{ct.color_text.BOLD} > eta edges {cat_p}{ct.color_text.END}')
        if args.debug : continue
        split_selection = f'((fabs(tau_fit_eta) > {cat_p[0]:,.1f}) && (fabs(tau_fit_eta) < {cat_p[1]:,.1f}))'
        sgn_selection       = '&'.join([
            base_selection, 
            split_selection,
        ])
        ...
        process_name = f'{process_name_base}_{cat}{j}{args.year}'
        tag = f'bdt{args.bdt_cut:,.4f}_{cat}{j}{args.year}_etalo{cat_p[0]:,.1f}_etahi{cat_p[1]:,.1f}' + (('_' + args.tag ) if not (args.tag is None) else '')
        print(f' > tag {tag}')
        ...
        # **** BACKGROUND MODEL ****
        # background PDF
        slope = ROOT.RooRealVar(f'slope_{cat}{j}{args.year}', f'slope_{cat}{j}{args.year}', -1.0, -10.0, 10.0)
        ...
        eta_lo.append(cat_p[0])
        eta_hi.append(cat_p[1])
        cat_id.append({'A':1, 'B':2, 'C':3}[cat])
        sig_Nexp.append(nsig.getValV())
        sig_eff.append(sig_efficiency)
        sig_width.append(width.getValV())
        sig_width_err.append(width.getError())
        ...
        wspace_tag      = f'{process_name_base}_bdt{args.bdt_cut:,.4f}_{cat}{j}{args.year}_etaAB{cat_p[0]:,.1f}_BC{cat_p[1]:,.1f}'
...
tree_dict = dict(zip(df_columns, np.array([bdt_cut, eta_lo, eta_hi, cat_id, sig_Nexp, sig_eff, sig_width, sig_width_err, bkg_Nexp, bkg_Nexp_Sregion, bkg_eff, PunziS_val, PunziS_err, AMS_val])))

...
def get_arguments():
    ...
    parser.add_argument('--plot_outdir',default= './plots/', help=' output directory for plots')
    ...
    parser.add_argument('--mu_pair',    choices=['1', '2', 'all'], default = 'all')
...
def optimize_vetoWindow(fitresults = {'x': None, 'nsig':None, 'mean': None, 'width':None}, sigma_min= 0.5, sigma_max=5.0, sigma_step = 0.5, outlog = None, thresold_events = 1.0):
    ...
    for n_sigma in sigma_thresholds:
        ...
        eff_signal = res_signal/nsig.getVal() if nsig.getVal() > 0 else 0.
        if outlog : message_logger(f' +/- {n_sigma:.1f} sigma: expected signal = {res_signal:.1f} (efficiency = {eff_signal*100:.2f} %)\n', outlog)
        
        #if res_signal < thresold_events: # optimal window -> less than 1 expected signal event
        if eff_signal < 0.01: # less than 1% signal efficiency
...
if __name__ == '__main__':
    ...
    args = get_arguments()
    tag = args.tag
    name = {'1': 'dimu_OS1', '2':'dimu_OS2', 'all':'dimu'}
    ...
    ctrl_bdt_lo, ctrl_bdt_hi = 0.00, 0.60
    ...
    # ** IMPORT DATA **
    input_tree_name = 'FinalTree'
    data_file = '/eos/user/c/cbasile/Tau3MuRun3/data/HF-test/AllData_novetoes_nov25_plusMVAANv7.root'
    ...
    base_selection = '&'.join([
        categ_dict[args.category],
        "year=="+args.year if args.year != 'all' else 'year>-1',
        "isMC==0"
        ])
    message_logger(f'[INFO] base selection: {base_selection}', logger)
    ctrl_selection = '&'.join([
        base_selection,
        f'(bdt_cv_ANv7 > {ctrl_bdt_lo} && bdt_cv_ANv7 < {ctrl_bdt_hi})'
        ])
    message_logger(f'[INFO] CONTROL region selection: {ctrl_selection}', logger)
    signal_selection = '&'.join([
        base_selection,
        f'(bdt_cv_ANv7 > {args.bdt_cut})'
        ])

    # ** RooFit Variables ** 
    thevars = ROOT.RooArgSet()
    # BDT score
    bdt = ROOT.RooRealVar('bdt_cv_ANv7', 'bdt_cv_ANv7', 0.0,  1.0, '' )
    thevars.add(bdt)
    weight = ROOT.RooRealVar('combine_weight', 'combine_weight', 0.0, 10.0, '' )
    # eta
    eta = ROOT.RooRealVar('category_v2', 'category_v2' , 0,  3, '' )
    thevars.add(eta)
    # year ID
    year = ROOT.RooRealVar('year', 'year', 2022,  2025, '' )
    thevars.add(year)
    # isMC 
    isMC = ROOT.RooRealVar('isMC', 'isMC', 0,  3, '' )
    thevars.add(isMC)
    # di-muon mass
    if args.mu_pair == 'all':
        ref_var= 'dimu'
        ...
        mumu_mass.setRange('fit_range_signal', fit_range_lo+0.08, fit_range_hi-0.08)
        vars_red = ROOT.RooArgSet(mumu_mass)
        #vars_red.add(mumu_ref)
        datactrl  = ROOT.RooDataSet("datactrl", "datactrl", vars_red)
        datasig   = ROOT.RooDataSet("datasig", "datasig", vars_red)

        dimusel = f'& dimu_OS1>{fit_range_lo} & dimu_OS1<{fit_range_hi} & dimu_OS2>{fit_range_lo} & dimu_OS2<{fit_range_hi}'
        #CTRL
        data_tree.Draw(">>elist_ctrl", ctrl_selection + dimusel) 
        elist_ctrl = ROOT.gDirectory.Get("elist_ctrl")
        # Loop manually over the TTree
        for i in range(elist_ctrl.GetN()):
            entry_idx = elist_ctrl.GetEntry(i)
            data_tree.GetEntry(entry_idx)
            # Fill from dimu_OS1
            #mumu_ref.setVal(data_tree.dimu_OS1)
            mumu_mass.setVal(data_tree.dimu_OS1)
            datactrl.add(ROOT.RooArgSet(mumu_mass))
            #datactrl.add(ROOT.RooArgSet(mumu_ref))
            # Fill from dimu_OS2
            #mumu_ref.setVal(data_tree.dimu_OS2)
            mumu_mass.setVal(data_tree.dimu_OS2)
            datactrl.add(ROOT.RooArgSet(mumu_mass))
            #datactrl.add(ROOT.RooArgSet(mumu_ref))

        #SIGNAL
        data_tree.Draw(">>elist_sig", signal_selection + dimusel) 
        elist_sig = ROOT.gDirectory.Get("elist_sig")
        # Loop manually over the TTree
        for i in range(elist_sig.GetN()):
            entry_idx = elist_sig.GetEntry(i)
            data_tree.GetEntry(entry_idx)
            # Fill from dimu_OS1
            #mumu_ref.setVal(data_tree.dimu_OS1)
            mumu_mass.setVal(data_tree.dimu_OS1)
            datasig.add(ROOT.RooArgSet(mumu_mass))
            #datasig.add(ROOT.RooArgSet(mumu_ref))
            # Fill from dimu_OS2
            #mumu_ref.setVal(data_tree.dimu_OS2)
            mumu_mass.setVal(data_tree.dimu_OS2)
            datasig.add(ROOT.RooArgSet(mumu_mass))
        ...
        ref_var= f'dimu_OS{args.mu_pair}'
        mass_var = f'dimu_OS{args.mu_pair}'
        #mumu_ref = ROOT.RooRealVar(ref_var, 'm(#mu^{+}#mu^{-}) (dR'+f'{args.mu_pair})', 0.0, 10.0, 'GeV' )
        mumu_mass = ROOT.RooRealVar(mass_var, 'm(#mu^{+}#mu^{-}) (dR'+f'{args.mu_pair})', fit_range_lo,  fit_range_hi, 'GeV' )
        thevars.add(weight)
    ...
    print(f'\n {ct.GREEN}[INFO]{ct.END} dataset to fit, entries = {datactrl.numEntries()}')
    datasig.Print('v')
    ...
    smodel.fitTo(datasig, ROOT.RooFit.Range('fit_range_signal')) 
    # different normalizations
    nsig     = ROOT.RooRealVar('Ns', 'N signal', 0.8*datasig.sumEntries(), 0., datasig.sumEntries())
    nsig_ctrl = ROOT.RooRealVar('Ns_ctrl', 'Nctrl signal', 0.8*datactrl.sumEntries(), 0., datactrl.sumEntries())
    ...
    order = ROOT.RooArgList(a0)
    bmodel = ROOT.RooChebychev('bmodel', 'bmodel', mumu_mass, order)

    nbkg = ROOT.RooRealVar('Nb', 'N combinatorics', 0.5*datasig.numEntries(), 0., datasig.numEntries())
    ...
    nSigmaVeto = optimize_vetoWindow(
        ...
        sigma_min=0.4, sigma_max=5.0, sigma_step=0.2,
        outlog=logger,
        thresold_events=1.0,
    )
    ...
    draw_cms_style(
        frame=frame,
        lumi_args={'lumi': LumiVal_plots[args.year]},
    ...
    = WINDOW TO VETO = [{:.3f} , {:.3f}] GeV (+/- {:.0f} MeV)
    '''.format(
        ...
        significance,
        veto_lo, veto_hi, nSigmaVeto*width.getVal()*1000
    ), logger)

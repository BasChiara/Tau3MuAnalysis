...
columns_to_read = [ 'bdt_score', 'tau_mu12_M', 'tau_mu13_M', 'tau_mu23_M', 'tau_mu12_fitM', 'tau_mu13_fitM', 'tau_mu23_fitM', 'tau_fit_eta', 'year_id']
data_df = pd.DataFrame(data_rdf.AsNumpy(columns_to_read))

mass_data = pd.DataFrame()
for pair in ['12', '13', '23']:
    tmp = data_df[data_df['tau_mu'+pair+'_M'] > 0][['bdt_score', 'tau_mu'+pair+'_fitM', 'tau_mu'+pair+'_M', 'tau_fit_eta', 'year_id']].copy()

...
sys_sources = {
    'CMS_pileup'                     : [['PU_weight'], 1],
    'CMS_mc_stat'                    : [[''], 0],
    'CMS_eff_m_trigger_2mu'          : [['tau_DoubleMu4_3_LowMass_SF'], 1],
    'CMS_eff_m_trigger_3mu'          : [['HLT_isfired_Tau3Mu'],2],
    'CMS_eff_m_id'                   : [['tau_mu1_IDrecoSF', 'tau_mu2_IDrecoSF','tau_mu3_IDrecoSF'], 1],
    'w_pt_eta_reweighting'           : [['NLO_weight'], 1],
    'w_pt_reweighting'               : [['pTV_weight'], 2], # ?? name ??    
}
...
def correction_sys(input_file, tree_name, selection, category, year):
    ...
    for sy_src in sys_sources:
        ...
        # -- correlation degree -- #
        corr = ''
        if   sys_sources[sy_src][1] == 0: corr = f'{category}_20{year}'
        elif sys_sources[sy_src][1] == 1: corr = f'20{year}'

        # -- apply systematics -- # 
            # MC stat systematics
        if sy_src == 'CMS_mc_stat':
            S_nominal = rdf.Count().GetValue()
            f_tot = 1. / np.sqrt(S_nominal) + 1.
            # HLT_Tau3Mu systematics
        elif sy_src == 'CMS_eff_m_trigger_3mu':

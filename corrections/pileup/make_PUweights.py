...
for j, jfile in enumerate(central_GoldenJson[era]):
    cmd = f'./pileupCalc_fix.py -i {jfile} --inputLumiJSON {central_PuJson[era]} --calcMode true --minBiasXsec 69200 --maxPileupBin 100 --numPileupBins 100  pileup_{era}_{j}.root'
    print(f'\n> {cmd}')
    os.system(cmd)
...
#  PU used for simulation (2023) -> ambiguous files --> retrive from nanoAOD
if '2022' in era:
    ...
    nano_chain.Draw('Pileup_nTrueInt>>h_mc_PU', '', 'goff')

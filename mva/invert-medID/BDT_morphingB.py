...
LOAD_MODEL = False # load model from file
MUON_TO_INVERT_ = 'mu2mu3' # 'mu3' or 'mu2mu3'

def loadInChunks(file_path, treename, selection, columns, chunk_size=100000):
    ...
    for start in range(0, total_rows, chunk_size):
        ...
        chunk_df = pd.DataFrame(chunk_rdf.AsNumpy(columns)) if columns else pd.DataFrame(chunk_rdf.AsNumpy())
        yield chunk_df

if __name__ == '__main__':

    '''
    usage : python BDT_morphingB.py <year> <muon-to-invert[mu3, mu2mu3]>
    '''
    ...
    proxy_sample  = f'/eos/user/c/cbasile/Tau3MuRun3/data/bkg_samples//XGBout_invMedID_DATA_{year}_invID-{MUON_TO_INVERT_}.root'
    ...
    print(f"[+] Proxy sample: {proxy_sample}")
    # ---- OUTPUT ---- #
    out_tag = f'-{MUON_TO_INVERT_}_nT15k_invIDopen'
    output_sample = os.path.join(
        ...
        f'XGBout_invMedIDandSideBands_DATA_{year}_invID{out_tag}_reweight.root'
    )
    ...
    if SAVE_MODEL and not LOAD_MODEL:
        model_file = os.path.join(os.path.dirname(proxy_sample), f'xgbBDTmodel_{year}_invID{out_tag}.json')
    ...
    colums_to_load = config.features + ['bdt_score', 'tau_fit_eta', 'tau_fit_mass', 'tau_mu1_MediumID', 'tau_mu2_MediumID', 'tau_mu3_MediumID', 'year_id']
    # ---- LOAD DATA ---- #
    treename = 'tree_w_BDT'
    target_df_chunks = []
    [target_df_chunks.append(chunk) for chunk in loadInChunks(target_sample, treename, real_data_selection, colums_to_load)]
    proxy_df_chunks  = []
    [proxy_df_chunks.append(chunk) for chunk in loadInChunks(proxy_sample, treename, base_selection, colums_to_load)]

    target_df = pd.concat(target_df_chunks, ignore_index=True)
    proxy_df  = pd.concat(proxy_df_chunks, ignore_index=True)

    Ntarget = len(target_df)
    Nproxy  = len(proxy_df)
    ...
    input_features = config.features + ['tau_fit_eta', 'bdt_score']
    if not LOAD_MODEL:
        print(f"  --- TRAINING --- ")
        # balance the dataset
        if frac > 1.0:
        ...
        parameters = {
                ...
                'subsample'             :  0.75,     # subsample ratio of the training instance at every boosting iteration
        ...
        model_to_load = f'/eos/user/c/cbasile/Tau3MuRun3/data/bkg_samples/xgbBDTmodel_{year}_invID-{MUON_TO_INVERT_}_nT15k_invIDopen.json'
        # load model
    ...
    proba = model.predict_proba(full_df[input_features], iteration_range = (0, best_iter +1))[:, 1]
